...
def create_dl_dt_plot(time, dl_dt, title="Commande dL/dt"):
    """
    ...
    fig.update_layout(
        ...
        yaxis=dict(range=[-float(n), float(n)]),
        title=title,
        hovermode='x unified',
        width=600,
        height=400
...
def create_tension_vs_target_plot(
    ...
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=time, y=t_rupture, mode="lines", name="Tension rupture"))
    fig.add_trace(go.Scatter(x=time, y=t_cible, mode="lines", name="Tension cible"))
    fig.add_trace(go.Scatter(x=time, y=t_boat, mode="lines", name="Tension bateau"))
    fig.add_trace(go.Scatter(x=time, y=t_rov, mode="lines", name="Tension ROV"))
    fig.add_trace(
        go.Scatter(
            x=time,
            y=t_max,
