...
class PlotlyWidget(QWidget):
    ...
    def update_figure(self, fig):
        ...
        # Forcer la recréation si le nombre de traces a changé ou si c'est la première mise à jour
            ...
            for i, trace in enumerate(fig.data):
                ...
                if hasattr(trace, 'type') and trace.type == 'scatter':
                    if hasattr(trace, 'x') and hasattr(trace, 'y'):
                        ...
                        # Inclure hovertext et hoverinfo si disponibles (nécessaire pour les tooltips)
                ...
                if (typeof Plotly !== 'undefined') {{
                    ...
                    for (var j = 0; j < updateData.length; j++) {{
                        if (updateData[j] !== null && j < scatterIndices.length) {{
                            ...
                            // Ajouter hovertext et hoverinfo si disponibles
