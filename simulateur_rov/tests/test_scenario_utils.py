...
def test_commande_scenario_progression_et_reset():
    ...
    assert commande_scenario("Fx_rov", scen, t=0, L=0, p=0) is None
    assert commande_scenario("Fx_rov", scen, t=1, L=0, p=0) == 1.0
    assert commande_scenario("Fx_rov", scen, t=2, L=0, p=0) is None
    assert commande_scenario("Fx_rov", scen, t=6, L=0, p=0) == 2.0
    assert commande_scenario("Fx_rov", scen, t=7, L=0, p=0) is None

    assert commande_scenario("Fx_rov", scen, t=0, L=0, p=0) is None
    assert commande_scenario("Fx_rov", scen, t=1, L=0, p=0) == 1.0

    scen_long = "(l>10:3)"
    assert commande_scenario("dL_dt", scen_long, t=0, L=0, p=0) is None
    assert commande_scenario("dL_dt", scen_long, t=1, L=12, p=0) == 3.0
    assert commande_scenario("Fx_rov", scen_long, t=1, L=12, p=0) == 3.0

    scen_vx = "(t>0:4)"
    assert commande_scenario("Vx_bateau", scen_vx, t=0, L=0, p=0) is None
    assert commande_scenario("Vx_bateau", scen_vx, t=1, L=0, p=0) == 4.0


def test_commande_scenario_param_commande_invalide():
    with pytest.raises(SystemExit):
        commande_scenario("BadParam", "(t>0:1)", t=1, L=0, p=0)
