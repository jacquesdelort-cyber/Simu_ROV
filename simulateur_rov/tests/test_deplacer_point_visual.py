...
def generate_deplacer_point_report(output_file: str | Path | None = None) -> Path:
    ...
    for idx, case in enumerate(cases):
        ...
        # Ajuster les axes pour chaque cas, avec la même échelle en x et y
        ...
        half_span = 0.7 * span  # marge de 30 %
        x_min = cx - half_span
        x_max = cx + half_span
        y_min = cy - half_span
        y_max = cy + half_span
    ...
    if output_file is None:
        output_file = Path("results") / "deplacer_point_visual_tests.html"
