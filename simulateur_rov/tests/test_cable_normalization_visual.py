...
def generate_visual_report(output_file: str | Path | None = None) -> Path:
    ...
    n_cases = len(cases)
    n_cols = 2
    n_plot_rows = (n_cases + n_cols - 1) // n_cols
    ...
    fig = make_subplots(
        ...
        + [[{"type": "scatter"}, {"type": "scatter"}] for _ in range(n_plot_rows)],
        subplot_titles=[""] + subplot_titles,
        horizontal_spacing=0.1,
        vertical_spacing=0.12,
        row_heights=[0.42] + [1.0] * n_plot_rows,
    )
    ...
    for idx, case in enumerate(cases):
        ...
        x_after, y_after = solver._normalize_cable_length(x_before, y_before, case.l_target)
        ...
        fig.add_trace(
            go.Scatter(
                ...
                marker=dict(size=7),
                hovertemplate="x=%{x:.3f} m<br>y=%{y:.3f} m<extra>Avant</extra>",
            ),
            row=row,
            col=col,
        )
        fig.add_trace(
            go.Scatter(
                ...
                marker=dict(size=7),
                hovertemplate="x=%{x:.3f} m<br>y=%{y:.3f} m<extra>Apres</extra>",
            ),
            row=row,
            col=col,
        )
        ...
        stats = (
            _stats_text("Avant", x_before, y_before)
            + "<br><br>"
            + _stats_text("Apres", x_after, y_after)
            + f"<br><br>L cible = {case.l_target:.4f} m"
        )
        fig.add_annotation(
            xref=f"x{idx + 1}" if idx > 0 else "x",
            yref=f"y{idx + 1}" if idx > 0 else "y",
            x=x_min + 0.02 * max(x_max - x_min, 1.0),
        ...
        go.Table(
            ...
            cells=dict(
                values=[
                    ...
                    [f"{summary.l_target:.4f}" for summary in summaries],
                    [f"{summary.l_before:.4f}" for summary in summaries],
                    [f"{summary.ds_min_before:.4f}" for summary in summaries],
                    [f"{summary.ds_max_before:.4f}" for summary in summaries],
                    [f"{summary.l_after:.4f}" for summary in summaries],
                    [f"{summary.ds_min_after:.4f}" for summary in summaries],
                    [f"{summary.ds_max_after:.4f}" for summary in summaries],
                ],
                fill_color="white",
                align="left",
                font=dict(size=10),
    ...
    fig.update_layout(
        title=(
            ...
            "<br><sup>Tableau recapitulatif global + comparaison avant/apres avec longueurs totales et min/max des segments</sup>"
        ),
        template="plotly_white",
        height=max(420 * n_plot_rows + 420, 1100),
        width=1200,
        hovermode="closest",
    )
